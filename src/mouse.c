...
extern int gstrftime __PROTO((char* buf, int bufsz, char* fmt, double l_clock));
extern void do_string __PROTO((char* s));
extern int set_label_tag __PROTO((void)); /* from set.c */

#include "command.h"
#include "mouse.h"
#include "mousecmn.h"

/* defined in set.c, use the border linestyle
 * for plotting temporary annotations */
...
*/
TBOOLEAN setting_zoom_region = FALSE;  /* flag, TRUE while user is outlining the zoom region */
int setting_zoom_x, setting_zoom_y;    /* coordinates of the first corner of the zoom region, in the internal coordinate system */
...
extern struct surface_points *first_3dplot;		/* from plot3d.c */
extern int plot3d_num;		   			/* from plot3d.c */
...
MousePosToGraphPosReal( int xx, int yy, double *x, double *y, double *x2, double *y2 )
{
    if (!is_3d_plot) {
#if 0
	printf("POS: xleft=%i, xright=%i, ybot=%i,
	    ytop=%i\n",xleft, xright, ybot, ytop);
#endif
	if (xright==xleft) *x = *x2 = 1e38; /* protection */
	else {
	    *x = min_array[FIRST_X_AXIS] + ((double)xx-xleft) / (xright-xleft) * (max_array[FIRST_X_AXIS]-min_array[FIRST_X_AXIS]);
	    *x2 = min_array[SECOND_X_AXIS] + ((double)xx-xleft) / (xright-xleft) * (max_array[SECOND_X_AXIS]-min_array[SECOND_X_AXIS]);
	}
	if (ytop==ybot) *y = *y2 = 1e38; /* protection */
	else {
	    *y = min_array[FIRST_Y_AXIS] + ((double)yy-ybot) / (ytop-ybot) * (max_array[FIRST_Y_AXIS]-min_array[FIRST_Y_AXIS]);
	    *y2 = min_array[SECOND_Y_AXIS] + ((double)yy-ybot) / (ytop-ybot) * (max_array[SECOND_Y_AXIS]-min_array[SECOND_Y_AXIS]);
	}
#if 0
  ...
  */
  ...
  /* Now take into account possible log scales of x and y axes */
    if  (is_log_x) *x = exp( *x * log_base_log_x );
    if  (is_log_y) *y = exp( *y * log_base_log_y );
    if  (is_log_x2) *x2 = exp( *x2 * log_base_log_x2 );
...
GetAnnotateString(char *s, double x, double y, int mode, char* fmt)
{
	...
	mode==MOUSE_COORDINATES_XDATETIME ||
	mode==MOUSE_COORDINATES_TIMEFMT) { /* time is on the x axis */
	char buf[0xff];
	char format[0xff] = "[%s, ";
	strcat(format, mouse_setting.fmt);
	strcat(format, "]");
...
#define SEC_OFFS_SYS 946684800
	...
	case MOUSE_COORDINATES_TIMEFMT:
	    ...
	    break;
	default: sprintf(b, mouse_setting.fmt, x);
    }
    return b;
}
...
void do_zoom ( double xmin, double ymin, double x2min, double y2min, double xmax, double ymax, double x2max, double y2max )
{
    ...
    z->x2min = x2min; z->y2min = y2min;
    z->xmax = xmax; z->ymax = ymax;
    z->x2max = x2max; z->y2max = y2max;
    apply_zoom( z );
}
...
#else
	sp = GetAnnotateString(s0, real_x, real_y,
	    mouse_mode, mouse_alt_string);
#endif
	if (ruler.on) {
#if 0
	    MousePosToGraphPosReal(ruler.px, ruler.py, &ruler.x,
		&ruler.y, &ruler.x2, &ruler.y2);
#endif
...
#if 0
	    MousePosToGraphPosReal(ruler.px, ruler.py,
		&ruler.x, &ruler.y, &ruler.x2, &ruler.y2);
#endif
	    if (TICS_ON(xtics))
...
builtin_toggle_border(struct gp_event_t *ge)
{
    ...
    if (is_3d_plot) {
	if (draw_border == 4095) do_string("set border; replot");
	else do_string("set border 4095 lw 0.5; replot");
    } else {
	if (draw_border == 15) do_string("set border; replot");
	else do_string("set border 15 lw 0.5; replot");
    }
    return (char*) 0;
}
...
builtin_nearest_log(struct gp_event_t *ge)
{
	...
	if (is_log_z) do_string("unset log z; replot");
	else do_string("set log z; replot");
    } else {
...
builtin_toggle_polardistance(struct gp_event_t *ge)
{
    ...
    if (display_ipc_commands()) {
	fprintf(stderr,
	    "distance to ruler will %s be shown in polar coordinates.\n",
...
void event_buttonpress(struct gp_event_t *ge)
{
	...
	if (!setting_zoom_region) {
		...
		MousePosToGraphPosReal(setting_zoom_x, setting_zoom_y,
		    &xmin, &ymin, &x2min, &y2min);
		xmax = real_x;
		x2max = real_x2;
		ymax = real_y;
		y2max = real_y2;
...
printf("MOUSE.C: doublclick=%i, set=%i, motion=%i, ALMOST2D=%i\n",(int)doubleclick,(int)mouse_setting.doubleclick,(int)motion,(int)ALMOST2D);
	...
	if (b==1 && term->set_clipboard
		...
		GetAnnotateString(s0, real_x, real_y,
		    clipboard_mode, clipboard_alt_string);
	...
	if (term->set_cursor) term->set_cursor(0,0,0);
    }
...
event_motion(struct gp_event_t *ge)
{
	...
	if (button&(1<<1)) {	        
	    ...
	    if (surface_rot_x < 0)   surface_rot_x = 0;
	    if (surface_rot_x > 180) surface_rot_x = 180;
 	    surface_rot_z = rint(fmod(zero_rot_z - 360.0*mouse_x/term->xmax,360));
	    if (surface_rot_z < 0) surface_rot_z += 360;
	    redraw = TRUE;
...
event_modifier(struct gp_event_t *ge)
{
    ...
    if (modifier_mask==0 && is_3d_plot
	&& (button&((1<<1)|(1<<2))) && !needreplot) {
...
bind_display(char* lhs)
{
	...
	char fmt[] = " %-17s  %s\n";
	fprintf(stderr, "\n");
	fprintf(stderr, fmt, "<B1>", "print coordinates to clipboard using `clipboardformat`\n                    (see keys '3', '4')");
...
update_ruler(void)
{
    if (!term->set_ruler || !ruler.on) return;
    (*term->set_ruler)(-1,-1);
    recalc_ruler_pos();
    (*term->set_ruler)(ruler.px,ruler.py);
}
...
void send_gpPMmenu(FILE * PM_pipe)
{
    ...
    if (!PM_pipe || !mouseGnupmdrv) return;
    gpPMmenu.use_mouse = mouse_setting.on;
    if (zoom_now == NULL) gpPMmenu.where_zoom_queue = 0;
      else {
	gpPMmenu.where_zoom_queue = (zoom_now == zoom_head) ? 0 : 1;
	if (zoom_now->prev != NULL) gpPMmenu.where_zoom_queue |= 2;
	if (zoom_now->next != NULL) gpPMmenu.where_zoom_queue |= 4;
   }
