...
{
    if (p1->colorMode != p2->colorMode) return 1;
    if (p1->positive != p2->positive)   return 1;
    if (p1->cmodel != p2->cmodel)   return 1;
	...
	case SMPAL_COLOR_MODE_RGB:
	    if (p1->colorFormulae != p2->colorFormulae)  return 1;
	    if (p1->formulaR != p2->formulaR)  return 1;
	    if (p1->formulaG != p2->formulaG)  return 1;
	    if (p1->formulaB != p2->formulaB)  return 1;
	    /* if (p1->ps_allcF != p2->ps_allcF )  return 1; */
	    break;
        case SMPAL_COLOR_MODE_FUNCTIONS:
	    /* coarse check based on typed fnct definitions */
	    if (strcmp(p1->Afunc.definition, p2->Afunc.definition)) return 1;
	    if (strcmp(p1->Bfunc.definition, p2->Bfunc.definition)) return 1;
	    if (strcmp(p1->Cfunc.definition, p2->Cfunc.definition)) return 1;
	    break;
	case SMPAL_COLOR_MODE_GRADIENT: {
	    int i=0;
	    if (p1->gradient_num != p2->gradient_num )  return 1;
	    for( i=0; i<p1->gradient_num; ++i ) {
	        if (p1->gradient[i].pos != p2->gradient[i].pos) return 1;
		if (p1->gradient[i].col.r != p2->gradient[i].col.r) return 1;
		if (p1->gradient[i].col.g != p2->gradient[i].col.g) return 1;
		if (p1->gradient[i].col.b != p2->gradient[i].col.b) return 1;
	    }
	    break;
	}
    }
...
char *color_to_str( rgb_color col, char *buf )
{
...
void str_to_color( char *buf, rgb_color *col )
{
...
static int interpolate_color_from_gray( double gray, rgb_color *color )
{
	...
	color->b = col2->b;
        ...
        double dx=sm_palette.gradient[idx].pos-sm_palette.gradient[idx-1].pos;
...
void color_from_gray( double gray, rgb_color *color )
{
    ...
    if (sm_palette.colorMode == SMPAL_COLOR_MODE_GRAY ) return;

    /* transform to RGB if necessary */
    switch( sm_palette.cmodel ) {
      case C_MODEL_RGB:  break;
      case C_MODEL_HSV:  HSV_2_RGB( color );  break;
...
{
  /* mid is maximum */
  if( left.r < mid.r && mid.r > right.r )  return 1;
  if( left.g < mid.g && mid.g > right.g )  return 1;
  if( left.b < mid.b && mid.b > right.b )  return 1;
  /* mid is minimum */
  if( left.r > mid.r && mid.r < right.r )  return 1;
  if( left.g > mid.g && mid.g < right.g )  return 1;
  if( left.b > mid.b && mid.b < right.b )  return 1;
...
static void HSV_2_RGB( rgb_color *col )
{
    ...
    t = v * (1.0 - s*(1.0-f));
    switch( i%6 ) {
      case 0:   col->r = v;   col->g = t;  col->b = p;  break;
      case 1:   col->r = q;   col->g = v;  col->b = p;  break;
      case 2:   col->r = p;   col->g = v;  col->b = t;  break;
      case 3:   col->r = p;   col->g = q;  col->b = v;  break;
      case 4:   col->r = t;   col->g = p;  col->b = v;  break;
      default:  col->r = v;   col->g = p;  col->b = q;  break;
    }
}
#undef CONSTRAIN
