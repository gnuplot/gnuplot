#ifndef lint
static char *RCSid() { return RCSid("$Id: datafile.c,v 1.212.2.38 2014/05/11 20:24:09 sfeam Exp $"); }
#endif
...
plot_option_using(int max_using)
{
    ...
    /* Note: This was left out by mistake in versions 4.6.0 and 4.6.1 */
    if (!END_OF_COMMAND && isstring(c_token)) {
	df_format = try_to_get_string();
	if (!valid_format(df_format))
...
df_readascii(double v[], int max)
{
	...
	/*{{{  copy column[] to v[] via use[] */
	{
		...
		if (use_spec[output].at) {
			...
			v[output] = real(&a);
			if (isnan(v[output]))
			    return DF_UNDEFINED;	/* 27 May 2014 */
		    }
