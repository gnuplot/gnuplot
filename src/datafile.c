#ifndef lint
static char *RCSid() { return RCSid("$Id: datafile.c,v 1.67 2004/11/12 07:14:52 sfeam Exp $"); }
#endif
...
#define NOTSEP (*s != df_separator)
...
#ifdef EAM_DATASTRINGS
	/* Keep pointer to start of this token if user wanted it for anything */
	for (i=0; i<MAXDATACOLS; i++) {
	...
	/* Trap string-value columns before trying to interpret them as numeric */
	/* FIXME EAM - At this point we've marked the column DF_GOOD if anyone  */
	/* wanted it as a string, but it will get marked DF_BAD below if it     */
	/* doesn't also parse as a number. Is this a problem???                 */
#endif
...
df_read_matrix(int *rows, int *cols)
{
	...
	*cols = c;
	...
	if (*rows > max_rows) {
	    ...
	    linearized_matrix = gp_realloc(linearized_matrix, (*cols) * max_rows * sizeof(float), "df_matrix");
	}
	/* store data */
	{
	    int i;
...
/*}}} */
    ...
    /* Make information about whether the data forms a grid or
     * not available to the outside world.
     */
    df_matrix = ( df_matrix_file
		  || ( (df_num_bin_records == 1) && ((df_bin_record[0].cart_dim[1] > 0) || (df_bin_record[0].scan_dim[1] > 0)) ) );

    /* Same idea, but try removing this one.  I can't see why
     * it is important for the rest of the program to know if
     * if the data came from a binary file. (DJS 20 Aug 2004)
     */
...
#ifdef BINARY_DATA_FILE
		if (df_binary_file || df_matrix_file) {
		    /* Scan through the tokens looking for largest column reference. */
		    int j;
		    for (j=c_token; !equals(j, ")") && !(j >= num_tokens || equals(j,";")); j++) {
			...
			if (equals(j, "$")) {
				...
				int_error(c_token, "$ must be followed by a number");
			}
		    }
		}
#endif
...
#endif
{
...
#ifdef EAM_DATASTRINGS
	/*
	 * If we are supposed to read plot or key titles from the first line
	 * of the data then do that and nothing else.
	 */
	    if (column_for_key_title != NO_COLUMN_HEADER) {
		df_datum--;
		if (!(*df_key_title)) {
		...
		FPRINTF((stderr,"df_readline: Found key title in col %d %s\n",
	...
	/*{{{  copy column[] to v[] via use[] */
	{
		...
		if (use_spec[output].expected_type >= CT_XTICLABEL) {
			...
			default:
			case CT_XTICLABEL:	axis = FIRST_X_AXIS;  axcol = 0; break;
			case CT_X2TICLABEL:	axis = SECOND_X_AXIS; axcol = 0; break;
			case CT_YTICLABEL:	axis = FIRST_Y_AXIS;  axcol = 1; break;
			case CT_Y2TICLABEL:	axis = SECOND_Y_AXIS; axcol = 1; break;
		    ...
		    /* But really we need to handle general case of implicit column 0 */
		    if (output == 1)
	 ...
	 */
#ifdef EAM_DATASTRINGS
	/* FIXME EAM - In theory it might be useful for the caller to know    */
	/* whether or not tic specs were read from this line, but all callers */
	/* would have to be modified to deal with it one way or the other.    */
...
#define MAX_FILE_EXT_LEN 10
	...
	if ( almost_equals(c_token, "file$type") || ((df_bin_filetype >= 0) && (!set_type)) ) {
	    int i;
	...
	*bin_record = (df_binary_file_record_struct *) gp_realloc(*bin_record, new_number * sizeof(df_binary_file_record_struct), "binary file data records");
	if (!*bin_record) {
	    *max_num_bin_records = 0;
	    int_error(c_token, "Error assigning memory for binary file data records");
	}
...
plot_option_array(void)
{
	...
	expecting_number = TRUE;  /* Set true in case user starts string with a comma. */

	/* If the user has no space between 'x' or 'X' and number,
	 * the parser creates a single token x#.  So, copy string
	 * and work with that rather than the tokens directly.
	 * Null terminate and point to empty string.
	 */
	c_token--;
	df_format[0] = '\0';
	...
	while (1) {
	    ...
	    if ( expecting_number && !(isdigit(*token_string) || !strncasecmp(token_string, "Inf", 3)) ) break;
	    ...
	    if ( !expecting_number && (isdigit(*token_string) || !strncasecmp(token_string, "Inf", 3)) ) {
#if 0
...
plot_option_comma_separated(df_comma_separated_type type, int arg)
{
	...
	if (test_val) {
	    char *cannot_flip_msg = "Cannot flip a non-existent dimension";
	    double dval;
		...
		c_token = token2double(c_token, &(df_bin_record[bin_record_count].cart_delta[arg]));
	    ...
	    case DF_FLIP:
		/* Set the direction of grid points increment in based upon letters for axes. */
		/* Check if there are any characters in string that shouldn't be. */
		copy_str(df_format, c_token, MAX_LINE_LEN);
		if (strlen(df_format) != strspn(df_format, "xXyYzZ"))
		    int_error(c_token, "Invalid character in dimension string. Only x, X, y, Y, z, or Z acceptable");
	    ...
	    case DF_SCAN:
		/* Set the method in which data is scanned from file.  Compare against
		 * a set number of strings.
		 */
		{
		int i;
		if ( !(df_bin_record[bin_record_count].cart_dim[0] || df_bin_record[bin_record_count].scan_dim[0]) || !(df_bin_record[bin_record_count].cart_dim[1] || df_bin_record[bin_record_count].scan_dim[1]) )
		    int_error(c_token, "Cannot alter scanning method for one-dimensional data");
		else
		    if (df_bin_record[bin_record_count].cart_dim[2] || df_bin_record[bin_record_count].scan_dim[2]) {
		...
		if (df_bin_record[bin_record_count].scan_skip[0] != dtemp)
		    int_error(c_token, "The number of bytes to skip must be an integer");
		if (df_bin_record[bin_record_count].scan_skip[0] < 0)
		    int_error(c_token, "The number of bytes to skip must be positive");
		break;
		}
		...
		c_token = token2tuple(c_token, tuple, 3);
		/* ????? Is there a precision constant for doubles similar to what is in limits.h for other types? */
		if ((tuple[0]*tuple[0] + tuple[1]*tuple[1] + tuple[2]*tuple[2]) < 10e-10)
	    ...
	    default:
		int_error(NO_CARET, "Internal error (datafile.c): Invalid comma separated type");
	    }
		...
		double tuple[3];
		token2tuple(c_token, tuple, 3);  /* Let the conversion routine find the syntax error. */
...
/* If the column number is greater than number of binary columns,
 * set the unitialized columns binary info to that of the last
 * specified column or the default if none were set.
 */
void
...
plot_option_binary_format(void)
{
    ...
    for (i=1; df_format[i] != '\0' && df_format[i] != '\"' && df_format[i] != '\'' && i <= MAX_LINE_LEN; ) {

	if (df_format[i] == ' ') {i++; continue;}  /* Ignore spaces. */
	...
	if (df_format[i] == '%') {
	    ...
	    /* Number of columns is less than a single digit, so check only a single digit.
	     * If the user enters more than one digit, the routine fails on the next pass.
	     */
	    if (isdigit(df_format[i])) {
		field_repeat = df_format[i] - '0';  /* Convert to a number.  Let zero be a valid entry. */
		i++;
	    }
	    else
		field_repeat = 1;
		...
		for (k=0,breakout=0; k < df_binary_tables[j].group_length; k++) {
			...
			if ( !strncmp(&df_format[i], df_binary_tables[j].group[k].name[m], strl) && strchr("%\'\" ",df_format[i+strl]) ) {
		...
		if (k == df_binary_tables[j-1].group_length) {
		    if (m == df_binary_tables[j-1].group[k-1].no_names) {
...
df_show_binary(FILE *fp)
{
	...
	fprintf(fp, "%s", df_bin_filetype_table[df_bin_filetype_default].extension);
    else
	fprintf(fp, "none");
...
df_readbinary(double v[], int max)
{
	...
	df_column = (df_column_struct *) gp_realloc(df_column, df_no_bin_cols * sizeof(df_column_struct), "datafile columns");
	df_max_cols = df_no_bin_cols;
    }

    /* In binary mode, the number of user specs was increased by the number of dimensions
     * in the underlying uniformly sampled grid previously.  Fill in those values.  Also,
     * compute elements of formula  x' = P*R*(x - c) + o
     */
	...
	/* P */
	translation_required |= rotation_matrix_3D(P, df_bin_record[df_bin_record_count].cart_p);

	/* R */
	translation_required |= rotation_matrix_2D(R, df_bin_record[df_bin_record_count].cart_alpha);

	if (df_matrix_file) {

	    /* Dimensions */
	    scan_size[0] = df_bin_record[df_bin_record_count].scan_dim[0];
	    scan_size[1] = df_bin_record[df_bin_record_count].scan_dim[1];

	    if (scan_size[0] == 0)
		int_error(NO_CARET, "Scan size of matrix is zero");

	    /* To accomplish flipping in this case, multiply the appropriate column
	     * of the rotation matrix by -1.
	     */
	    for (i=0; i < 2; i++) {
		int j;
		for (j=0; j < 2; j++) {
	    ...
	    /* o */
	    for (i=0; i < 3; i++) {
		if (df_bin_record[df_bin_record_count].cart_trans != DF_TRANSLATE_DEFAULT) {
		    o[i] = df_bin_record[df_bin_record_count].cart_cen_or_ori[i];
		} else {
	    ...
	    /* c */
	    for (i=0; i < 3; i++) {
		if (df_bin_record[df_bin_record_count].cart_trans == DF_TRANSLATE_VIA_ORIGIN) {
		    if (i < 2)
		...
		if (df_bin_record[df_bin_record_count].cart_dim[i] || df_bin_record[df_bin_record_count].scan_dim[i]) {
		    if (df_bin_record[df_bin_record_count].scan_generate_coord) {
			use_spec[i].column = df_bin_record[df_bin_record_count].cart_scan[i];
		    }
		}
		/* Dimensions */
		map = DF_SCAN_POINT-df_bin_record[df_bin_record_count].cart_scan[i];
		if (df_bin_record[df_bin_record_count].cart_dim[i] > 0)
		    scan_size[map] = df_bin_record[df_bin_record_count].cart_dim[i];
		else
		    scan_size[map] = df_bin_record[df_bin_record_count].scan_dim[map];
		/* Sample periods */
		if (df_bin_record[df_bin_record_count].cart_delta[i])
		    delta[map] = df_bin_record[df_bin_record_count].cart_delta[i];
		else
		    delta[map] = df_bin_record[df_bin_record_count].scan_delta[map];
		delta[map] = delta[map]*df_bin_record[df_bin_record_count].scan_dir[map]*df_bin_record[df_bin_record_count].cart_dir[i];
		/* o */
		if (df_bin_record[df_bin_record_count].cart_trans != DF_TRANSLATE_DEFAULT)
		    o[i] = df_bin_record[df_bin_record_count].cart_cen_or_ori[i];
		else if (df_bin_record[df_bin_record_count].scan_trans != DF_TRANSLATE_DEFAULT)
		    o[i] = df_bin_record[df_bin_record_count].scan_cen_or_ori[map];
		...
		/* c */
		if ( (df_bin_record[df_bin_record_count].cart_trans == DF_TRANSLATE_VIA_ORIGIN)
		     || ((df_bin_record[df_bin_record_count].cart_trans == DF_TRANSLATE_DEFAULT) && (df_bin_record[df_bin_record_count].scan_trans == DF_TRANSLATE_VIA_ORIGIN)) ) {
	...
	memory_data = df_bin_record[df_bin_record_count].memory_data;

	/* byte read order */
	read_order = byte_read_order(df_bin_file_endianess);

	/* amount to skip before record */
	record_skip = df_bin_record[df_bin_record_count].scan_skip[0];
	...
	while (record_skip) {
	    if (memory_data) {
		memory_data++;
	    } else if ((fread_ret = fread(&io_val.ch, sizeof(io_val.ch), 1, data_fp)) != 1) {
		if (feof(data_fp)) {
		    df_eof = 1;
		    return DF_EOF;
		} else
	...
	/* Bring in variables as described by the field parameters.  If less than
	 * than the appropriate number of bytes have been read, issue an error
	 * stating not enough columns were found.
	 */
	for (i=0; ; i++) {
		...
		} else if ((fread_ret = fread(&io_val.ch, sizeof(io_val.ch), skip_bytes, data_fp)) != skip_bytes) {
		    if (feof(data_fp)) {
			df_eof = 1;
			return DF_EOF;
		    } else
	    ...
	    /* Matrix file data is a special case. After reading in just one
	     * binary value, stop then decide on what to do with it. */
	    if (df_matrix_file)
		break;
	...
	if (df_matrix_file) {
		...
		if (!df_M_count && !df_N_count && !df_O_count) {
			...
			if (!first_matrix_row_col_count) {
			    if (! (scanned_matrix_row =
				   gp_realloc(scanned_matrix_row, scan_size[0]*sizeof(float), "gpbinary matrix row")) )
	    ...
	    /* Update all the binary columns.  Matrix binary and matrix ASCII is
	     * a slight abuse of notation.  At the command line, 1 means first row,
	     * 2 means first column.  There can only be one column of data input
	...
	if (df_current_index < df_lower_index || df_current_index > df_upper_index ||
	    ((df_current_index - df_lower_index) % df_index_step) != 0)
	    continue;
	/*}}} */
	...
	/* At this point the binary columns have been read successfully.  Set
	 * df_no_cols to df_no_bin_cols for use in the interpretation code.
	 */
	...
	/*{{{  copy column[] to v[] via use[] */
	{
		...
		if (use_spec[output].at) {
		    ...
		    v[output] = real(&a);
		} else if (column == -5) {      /* Perhaps try using a switch statement to avoid so many tests. */
