 ...
 * against. */
#ifndef TEST_QUADTREE
#define TEST_QUADTREE 0
#endif
#if TEST_QUADTREE && TEST_GRIDBOX
# warning TEST_QUADTREE & TEST_GRIDBOX do not work together, sensibly!
#endif
 ...
 * 
 * In principle, this routine belongs into graph3d, but is currently
 * only used in hidden3d, and having it here as a static one allows
 * for easier inlining by gcc -O3.  */
static void
map3d_xyz(x, y, z, v)
	...
	if (new_poly > -2) {
		...
		case 1:
			...
			if (((new_edge == old_edge)
				 ...
				 * 2) check wether it's in front of or behind the other polygon's
				 *    plane
				 * 3) if in front, color the edge accoring to the vertex' polygon,
				 *    otherwise, color like the other polygon */
  ...
  resize_dynarray(&edges, ne);
     ...
     * edges properly */
	    ...
	    for (i = 0; i < icrvs->p_count; i++) {
				...
				case LINES:
					if (i > 0) {
						/* not first point, so we might want to set up the edge(s)
						 * to the left of this vertex */
						if (thisvertex < 0) {
							if ((crv > 0)
									&& (hiddenShowAlternativeDiagonal)
									) {
								/* this vertex is invalid, but the other three might
								 * still form a valid triangle, facing northwest to do
								 * that, we'll need the 'wrong' diagonal, which goes
								 * from SW to NE: */
								these_edges[i*3+2] = e3
									= store_edge(vertices.end - 1, edir_NE, crvlen,
																lp, above);
								if (e3 > -2) {
									/* don't store this polygon for later: it doesn't
									 * share edges with any others to the south or east,
									 * so there's need to */
									pnum 
										= store_polygon(vertices.end - 1, pdir_NW, crvlen);
									/* The other two edges of this polygon need to be
									 * checked against the neighboring polygons'
						 ...
						 * check the other three of this cell, by trying to set up
						 * the edges from this one to there */
						these_edges[i*3] = e1
						...
						if (crv > 0) {			/* vertices to the north exist */
							...
							if (e3 > -2) {
								/* diagonal edge of this cell is OK, so try to build
								 * both the polygons: */
								if (e1 > -2) {
									/* one pair of edges is valid: put first polygon,
									 * which points towards the southwest */
								...
								if (e2 > -2) {
									/* other pair of two is fine, put the northeast
                   * polygon: */
								 ...
								 * orientation, find good coloring of the diagonal */
								...
								if (e3 > -2) {
									/* fill this polygon into *both* polygon places for
									 * this quadrangle, as this triangle coincides with
									 * both edges that will be used by later polygons */
									these_polygons[2*i] = these_polygons[2*i+1] = pnum
										= store_polygon(thisvertex, pdir_SE, crvlen);
									/* This case is somewhat special: all edges are new,
									 * so there is no other polygon orientation to
									 * consider */
...
#define setup_edge(vert1, vert2)			\
	...
	for (polynum = *firstpoly, p = plist + polynum;
		 ...
		 * triangle. Used both by macro 'checkside', and for individual
		 * checking, beloe --> put this into a macro */
...
#undef checkside								/* get rid of that macro, again */
     ...
     * the polygon's plane? */
		 ...
		 * this plane, and store into entry 'POLY_NVERT' of the
		 * classification array, following the three edges of the
		 * polygon. */
		...
		if ((v1_inside_p) && (v2_inside_p)) {
			...
			/* Check if edge really doesn't intersect the triangle (this
		 ...
		 * e_side[2][POLY_NVERT], p_side[POLY_NVERT]. Setting aside cases
		 * where one of them is zero, this leaves a total of 2^11 = 2048
		 * separate cases, in principal. Several of those have been sorted
		 * out, already, by the previous tests. That leaves us with 3*27
		 * cases, essentially. The '3' cases differ by which of the three
...

		{
			 ...
			 * intersection parameters, as appropriate. Start by regarding
			 * the polygon's plane: '1' means point is in front of plane.
			 * 
...
#define hit_in_edge(hit) ((hit >= 0) && (hit <= 1))
			 ...
			 * the shadow volume. A macro contains the code, to be used for
			 * both side1 and side2, to reduce code overhead: */
...
#undef check_out_hit
			...
			/* Macro: combine all the classifications into a single
...
#define makeclass(s2, s1, p) (16 * s2 + 4 * s1 + p)
			 ...
			 * more than one intersecting plane. Only one of those can be
			 * 'the real' intersection of the edge with the surface of the
			 * shadow volume surface. The others are intersections with the
			 * planes defining the volumes, but outside the actual faces.
			 *
			 * This cuts down the 27 to 13 cases (000, 001, 002, 012, and
			 * their permutations). Start with v1: */
			...
			case makeclass(1,0,1):
				...
				/* The following two cases never trigger, even in a full run
				 * through all.dem. Neither do the corresponding 2 cases for
				 * the vertex2 classification. I think this is so because the
				 * determination of 'side1' and 'side2' already took care of
				 * this: if the edge vertex is out two sides, those two will
				 * _not_ be chosen as 'side1' and 'side2', because one of them
				 * is completely on one side of the edge.  */
			case makeclass(1,1,0):
				/* v1 out both sides, but behind the front */
				if (hit1 < hit2) 
					/* hit2 is closer to the hidden vertex v2, so it's the relevant
					 * intersection: */
					classification[side1] = 0;
				else
					classification[side2] = 0;
			...
			case makeclass(2,0,2):
				...
				/* See note in v1 handling about these two cases being
				 * impossible... */
			...
			case makeclass(2,2,2):
				/* v2 out both sides, and in front of p (--> v1 is hidden) */
				if (front_hit > hit1) {
					classification[3] = 0;
					if (hit1 > hit2)
						classification[side1] = 0;
...
/*----------- cases with 2 certain intersections: --------------*/
			...
			case makeclass(2,1,0):
				 ...
				 * These two cases can be handled by common code: */
...
/*----------- cases with either 2 or no intersections: --------------*/
				...
				/* Mainly identical code block to be used 4 times --> macro */
#define handle_outside_behind_vs_infront(vert1, vert2, hitvar)				\
				/* vert1 out plane of 'hitvar' and in back, vert2 in front */	\
				if ((hitvar) < front_hit) {																		\
				...
				/* otherwise, the edge missed the shadow volume --> nothing		\
				 * to do */																										\
				break;
...
#undef handle_outside_behind_vs_infront
	...
	/* Came here, so there's something left of this polygon, which needs
