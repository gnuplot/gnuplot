#ifndef lint
static char *RCSid() { return RCSid("$Id: hidden3d.c,v 1.13 1999/11/03 16:05:34 lhecking Exp $"); }
#endif
...
static long int make_edge(vnum1, vnum2, lp, style, next)
...
# define SET_XEXTENT \
  xextent = (~COORD_TO_BITMASK(xmax,1)) & COORD_TO_BITMASK(xmin,0);
# define SET_YEXTENT \
  yextent = (~COORD_TO_BITMASK(ymax,1)) & COORD_TO_BITMASK(ymin,0);
#else
...
/* CODEME: second hit is ignored, for all of these: */
			...
			case makeclass(0,1,2):
				...
				if (hit1 < front_hit) {
					/* hit1 is closer to v1, front_hit is closer is closer to v2.
...
/*  					printf("--> T9: split e by side1 plane (case= %d)\n", full_class); */
			...
			case makeclass(0,2,1):
				...
				if (hit1 > front_hit) {
					/* hit1 is closer to v2, front_hit is closer is closer to v1.
					 * --> we have two intersection points... */
					newvert[0] = split_line_at_ratio(e, hit1);
					newvert[1] = split_line_at_ratio(e, front_hit);
/*  					printf("--> T9: split e by side1 plane (case= %d)\n", full_class); */
			...
			case makeclass(1,0,2):
				if (hit2 < front_hit) {
					/* hit2 is closer to v1, front_hit is closer is closer to v2.
					 * --> we have two intersection points... */
					newvert[0] = split_line_at_ratio(e, hit2);
					newvert[1] = split_line_at_ratio(e, front_hit);
/*  					printf("--> T9: split e by side1 plane (case= %d)\n", full_class); */
			...
			case makeclass(2,0,1):
				...
				if (hit2 > front_hit) {
					/* hit2 is closer to v2, front_hit is closer is closer to v1.
					 * --> we have two intersection points... */
					newvert[0] = split_line_at_ratio(e, hit2);
					newvert[1] = split_line_at_ratio(e, front_hit);
/*  					printf("--> T9: split e by side1 plane (case= %d)\n", full_class); */
