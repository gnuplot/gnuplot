    ...
    switch(line[0]) {        /* control character */
       ...
       case '@': {            /* start/end table */
                ...
                if( intable ) {
                    ...
                    tablecols = 0 ;
                    tableins = &table ;
                    for(j=0;j<MAX_COL;j++)tablewidth[j]=0 ;
                    } 
                else { /* dump table */
                    int header = 0 ;
                    intablebut = FALSE ;
                    ...
                    if(header)tableins->next=tableins->next->next;
                    while( tableins != NULL ) {
                            fprintf(b,":row.\n" ) ;
       ...
       case '%': {            /* troff table entry */
                if( intable ) {
                    if( introffheader ) {
                        ...
                        if( line2[1] == '.' ) break ;
                        pt = strchr( line2, '(' ) ;
                        if( pt != NULL ) tablechar = *(pt+1) ;
                        fprintf( stderr, "tablechar: %c\n", tablechar ) ; 
                        pt = strchr( line2+2, '.' ) ;
                        if( pt != NULL ) introffheader = FALSE ;
                        break ;
                        }
                    ...
                    for( j; j<MAX_COL; j++ ) tableins->col[j][0]='\0' ;        
                    }
                break;            /* ignore */
                }
       case '\n':            /* empty text line */
