...
%
% $Id: faq-ja.tex,v 1.29 2008-02-19 15:24:45+09 shige Exp $
% translated to Japanese by Shigeharu TAKENO 2005/03/14
% Original: Id: faq.tex,v 1.37 2008/01/24 18:13:04 sfeam Exp
%
%
...
この文書では、2007 年 3 月にリリースされた \gnuplot{} の正式最新版である %
version 4.2 を取り扱っています。
%% end Japanese
        \\
%%% Its version is
%%% \verb+Revision: 1.37 +, dated
%%% \verb+Date: 2008/01/24 18:13:04 +.
% <== 本当は \verb++ の内側に $ がついている
この文書のバージョンは %
\verb+$+\verb+Revision: 1.37 $+,
\verb+$+\verb+Date: 2008/01/24 18:13:04 $+ です。
...
%%% MS-DOS, Amiga, OS-9/68k, Atari ST, BeOS, and Macintosh. 
...
MS-DOS, Amiga, OS-9/68k, Atari ST, BeOS, Macintosh などの 
...
%%% The current released version of \gnuplot{} is 4.2 (March 2007).
%%% The current patchlevel is 4.2.2 (August 2007).
\gnuplot{} の最新の正式リリース版は 4.2 です (2007 年 3 月現在)。
最新のパッチレベルは 4.2.2 です (2007 年 8 月現在)。

% The current development series consists of a series of snapshots
% with version labels 4.3-<date>
...
%%% Older versions of the \gnuplot{} distribution is also mirrored
...
%%% on gnuplot's
%%% sourceforge site \http{gnuplot.sourceforge.net}.
...
%%% In MS Windows you click in the upper left
%%% corner of the graph window and print directly from there.
MS Windows ではグラフウィンドウの左上隅をクリックすると
それを直接プリントアウトします。
...
%%% The example below demonstrates this for two curves f(x) and g(x) 
%%% with a tricky
...
%%% but not, unfortunately, PostScript.  Update: The CVS version of gnuplot
...
更新情報: CVS 版の gnuplot では、PostScript を含むより完全な %
UTF-8 のサポートがなされています。
...
%%% postscript, pm,
%%% x11, png, pdf) allow you to change fonts and text sizes within 
%%% a plot. Look at
...
%%% documentation. You can find the version 4.2 examples at 
%%% \http{gnuplot.sourceforge.net/demo}.  Examples from the development version
%%% are at \http{gnuplot.sourceforge.net/demo_4.3}.
古いドキュメントに書かれている NASA のサイトからはサンプルは削除されました。
現在は version 4.2 用のサンプルが \http{gnuplot.sourceforge.net/demo} %
にあります。
開発版用のサンプルは \http{gnuplot.sourceforge.net/demo_4.3} %
にあります。
