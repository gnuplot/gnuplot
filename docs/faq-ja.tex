...
%
% $Id: faq-ja-4.6.tex,v 1.5 2012-03-05 14:52:15+09 shige Exp $
% translated to Japanese by Shigeharu TAKENO
% Original: Id: faq.tex,v 1.48 2012/03/02 17:50:40 sfeam Exp
%
% 注: gnuplot-4.6 には、1.48 (2012/03/02) の FAQ.pdf が同封される ?
% 
...
この文書では、2012 年 3 月にリリースされた \gnuplot{} の正式最新版である %
version 4.6 を取り扱っています。
%% end Japanese
        \\
%%% Its version is
...
\verb+$+\verb+Date: 2012/03/02 17:50:40 $+ です。
...
\gnuplot{} の最新の正式リリース版は 4.6 です。
最新のパッチレベルは 4.6.0 です (2012 年 3 月現在)。
...
他のソフトのインストールと同様です。\texttt{README.1ST}, \texttt{README}, 
\texttt{INSTALL} ファイルを読んでください。
...
Windows では、makefile は \texttt{config/mingw}, \texttt{config/msvc},
\texttt{config/watcom}, \texttt{config/cygwin} 内にあります。
makefile の先頭に書かれているオプションを修正し、
その makefile が置かれているのと同じディレクトリ内で
適当な \texttt{make} ツールを実行してください。
...
を見てください。そこには以下のものを生成するファイルがあります。
...
もしかしたら \texttt{automake}, \texttt{autoconf} というツールも
...
%%% on the
%%% command line.
\texttt{windows} 出力形式では、そのツールバーの上のプリンタアイコンを
クリックする、あるいはコマンドラインで \texttt{screendump} を実行することで
グラフをプリントアウトできます。
...
\\includegraphics コマンドを使って取り込むための *.eps, *.pdf, *.png 出力を
生成するものもあります。
...
そうでなければ、whirlgif 3.04 というツールを入手してください。
...
疑似ファイル '+' と \verb+filledcurves closed+ を使えば簡単にできます。
...
\texttt{help ternary} に関するドキュメントを参照してください。
\texttt{demos} ディレクトリ内の \texttt{fillbetween.dem} も参照してください。
...
%%% \verb+set table+.
他のやり方としては、\verb+set table+ を利用して等高線を
一時ファイルに書き出す手があります:
%% end Japanese
\begin{verbatim}
...
\texttt{set palette cubehelix} を試してください。
...
\subsection{\gnuplot{} 4.2, 4.4 等で新しくなったことは}
...
\gnuplot\ version 4.2 以降では、横並び型 (clustered) %
と積み上げ型 (stacked) のヒストグラムスタイルが、
別々のスタイル型としてサポートされています。
...
それは \gnuplot ではちょっと面倒です。以下を見てください。
\http{http://gnuplot-tricks.blogspot.com/2009/08/pie-charts-entirely-in-gnuplot.html}
または、以下を参照してください。
\http{www.usf.uni-osnabrueck.de/~breiter/tools/piechart/piecharts.en.html}
...
更新情報: バージョン 4.4 の gnuplot では、PostScript を含むより完全な %
UTF-8 のサポートがなされています。
...
"enhanced text" モードをサポートしている出力形式では
描画内でテキストサイズとフォントの変更が行なえます。
...
\texttt{fudgit} を試してみるといいでしょう。これは Martin-D.Lacasse によって
書かれた対話型多目的曲線当てはめプログラムです。
...
最後に、\texttt{scilab} も \http{www.scilab.org} にあります。
...
Windows では、これはコンソール版の \texttt{gnuplot} か、
テキストコンソールが追加された GUI 版の \texttt{wgnuplot\_pipes} %
のみがサポートします。
Cygwin 版の gnuplot も、パイプは当然サポートしています。
...
コンソール版の \texttt{gnuplot} (推奨) か、
テキストコンソールが追加された GUI 版の \texttt{wgnuplot\_pipes} %
のいずれかを使う必要があります。
古い \texttt{pgnuplot} というヘルパープログラムも
まだ配付ッケージに含まれています。
...
version 4.2, 4.4, 4.6 のリリースは Ethan Merritt が取りまとめました。
