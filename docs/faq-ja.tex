...
%
% $Id: faq-j.tex,v 1.10 2006-03-26 11:53:15+09 shige Exp $
% translated to Japanese by Shigeharu TAKENO 2005/03/14
% Original: Id: faq.tex,v 1.19 2006/01/19 09:57:54 mikulik Exp
%
%
...
\noindent
%% This document deals with \gnuplot{} version 4.0 which is the
%% latest official release as of April 2004.
この文書では、2004 年 4 月にリリースされた \gnuplot{} の正式最新版である %
version 4.0 を取り扱っています。
        \\
この文書のバージョンは %
\verb+$+\verb+Revision: 1.19 $+,
\verb+$+\verb+Date: 2006/01/19 09:57:54 $+ です。
...
%% GIF support was provided by an external library, libgd.
%% Version 1.6 of the gd library dropped support for gif in favour of the
%% superior png format. More recent versions of libgd also support jpeg
%% output.  If your installation of gnuplot is linked to the gd library,
%% you will get support for whatever formats (gif, png, jpeg) are in that
%% version of libgd.
GIF のサポートは外部のライブラリである libgd で与えられていましたが、
Version 1.6 の gd ライブラリは、より優れている png 形式に味方して、
gif のサポートをやめてしまいました。
より最近の libgd では jpeg 出力もサポートされています。
もしその gd ライブラリをリンクして gnuplot をインストールしたのであれば、
その版の libgd ではいずれの形式 (gif, png, jpeg) も含まれているので、
それらはいずれもサポートされています。

%% Thus, nowadays, the usual way is to output the image as png and then use your
%% favourite program to convert it into gif, either using command line (e.g.
%% \verb+convert f.png f.gif+ or \verb+nconvert -out gif f.png+) or by any GUI
%% program. Another possibility is to output the image as (encapsulated) 
%% postscript
%% and convert (export) it into gif or png by ghostscript, e.g.
%% \verb+convert -density 150 f.eps f.gif+ or by any ghostscript-based GUI 
%% like gsview,
%% gv or kghostview.
よって、現在の一般的な方法は、png 画像を出力させ、
それを適当なプログラムを使って、
コマンドラインから %
(例えば \verb+convert f.png f.gif+ や \verb+nconvert -out gif f.png+ 等)、
あるいは GUI ツールによって変換させることでしょう。
...
そして、\http{www.danbbs.dk/~dino/whirlgif} にある %
...
%% Please refer to the \texttt{NEWS} file in the source distribution, or the News
%% section in the gnuplot documentation.
ここに記すには多すぎます。
ソース配布物に含まれるファイル \texttt{NEWS}、
あるいは gnuplot のヘルプドキュメントの News のセクションを参照してください。
...
Version 3.7 からすべてのプラットホームで隠線処理がサポートされました。
\verb+set hidden3d+ コマンドを使ってください。
...
\gnuplot{} の 16-bit バイナリは 640KByte のメモリ制限のため、
部分的にのみ隠線処理をサポートしています。


%% \subsection{Does \gnuplot{} support bar-charts/histograms/boxes?}
\subsection{\gnuplot{} は棒グラフ/ヒストグラム/箱型グラフが書けますか}

%% Use the style \verb+with boxes+ for bar charts. To get filled boxes,
%% use \verb+set style fill+.
棒グラフは \verb+with boxes+ スタイルを使ってください。
...
%% There is a large patch (\#606169) against the development version 
%% on SourceForge
%% awaiting inclusion in a future release of \gnuplot{}. This adds a new
%% plotting style \verb+with histograms+, and supports both clustered histograms
%% and stacked histograms.
将来 \gnuplot{} のリリース版に含まれる SourceForge の開発版には
大きなパッチ (\#606169) が含まれています。
これは、新しい描画スタイル \verb+with histograms+ を追加するもので、
横並び型のヒストグラムと積み上げ型のヒストグラムの両方をサポートします。
...
\item \texttt{barchart\_via\_fig}: 塗りつぶした棒グラフを生成する %
awk スクリプト
...
πのようにあらかじめ定義されている変数としてはありません。
しかしラベルにプランク定数 (h に横線) の記号を出力したいならば %
PostScript terminal を使う必要があります。
次のようにすればそれを出力できます: 
...
もう一つの方法は %
\verb+{/=14 @^{/Symbol=10 -}{/=14 h}}+ とすることです。

%% (This is a hint by Richard Crawford).
(Richard Crawford のヒントによります)
