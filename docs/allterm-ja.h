...
"?term aqua",
"?aqua",
" この出力形式は MacOS 上の表示に関する AquaTerm.app に依存しています。",
"",
" 書式:",
"       set terminal aqua {<n>} {title \"<wintitle>\"} {size <x> <y>}",
"                         {font \"<fontname>{,<fontsize>}\"}",
...
"",
" The size of the plot is given in screen pixels.  By default (`anchor`) each",
" plot is drawn at the top left of the terminal window.  This is useful for",
" animation or pseudo-mousing using the keyboard (see `pseudo-mousing`).",
" `scroll` instead starts each plot at the current cursor position so that it",
" scrolls with the terminal text.",
""
#else /* JAPANESE_DOC */
"1 kittycairo",
"?set terminal kittycairo",
...
"?term kittycairo",
"?kittycairo",
" 出力形式 `kittycairo` は、kitty グラフィックプロトコルをサポートする端",
" 末エミュレータ上で、ウィンドウ内出力を生成します。実際の描画は、2 次元",
" グラフィックライブラリの cairo と文字列レンダリングライブラリの pango",
" を用いて行います。kitty プロトコルは、sixel グラフィックとは別の選択肢",
" で、24 ビット RGB カラーをサポートすること、およびリモートセッションで",
" はコンピュータと端末の間の画像データの転送に必要な大域幅が少し小さいこ",
" とが優位です。",
"",
" 書式:",
"         set terminal kittycairo",
...
"                      {size <XX>,<YY>} {anchor|scroll}",
"",
" この出力形式は、ラベルや他の文字列処理にデフォルトで拡張文字列処理を使",
" 用します。以下参照: `enhanced`。",
"",
" グラフのすべての線分の幅は、`linewidth` の積因子 <lw> で変更できます。",
" フォントサイズも同様に `fontscale` のスケール積因子で一様に変更できま",
" す。フォントや文字コードオプションに関する説明については、以下参照:",
" `pdfcairo`。",
"",
" オプション `rounded` は、線の端や接合部を丸くし、デフォルトの `butt`",
" は尖った端と角張った接合部を生成します。",
"",
" グラフのサイズは、スクリーンピクセル単位で与えます。デフォルトでは",
" (`anchor`)、それぞれのグラフは、端末ウィンドウの左上に描きます。これは",
" アニメーションや、キーボードを利用した疑似マウス操作 (以下参照:",
" `pseudo-mousing`) では便利です。一方、`scroll` では、各グラフを現在の",
" カーソル位置から開始しますが、それにより端末文字列でそれをスクロールで",
" きます。",
""
#endif /* JAPANESE_DOC */
END_HELP(kittycairo)
...
"                    {background <rgb_color>}",
"",
" 出力形式 `kittygd` は、kitty グラフィックプロトコルをサポートする端末",
" エミュレータ上で、ウィンドウ内出力を生成します。デフォルトでは、ライブ",
" ラリは 24 ビット RGB PNG 画像を生成し、出力時に 256 色 (`truecolor`)",
" 減色変換されます。`notruecolor` は、出力の色をさらに少なく制限しますが",
" これによる明確な優位性はありません。",
" 透過型の fill style にはオプション `truecolor` が必要です。以下参照:",
" `fillstyle`。",
" あなたの gnuplot が、cairo グラフィックスをサポートするようにビルドさ",
" れたものなら、むしろ `kittycairo` の方がいいでしょう。",
"",
" この出力形式は、ラベルや他の文字列処理にデフォルトで拡張文字列処理を使",
" 用します。以下参照: `enhanced`。",
"",
...
" しますが、より遅くなります。",
"",
" グラフのサイズは、スクリーンピクセル単位で与えます。",
" デフォルトでは (`anchor`)、それぞれのグラフは、端末ウィンドウの左上に",
" 描きます。これはアニメーションや、キーボードを利用した疑似マウス操作",
" (以下参照: `pseudo-mousing`) では便利です。一方、`scroll` では、各グラ",
" フを現在のカーソル位置から開始しますが、それにより端末文字列でそれをス",
" クロールできます。",
""
#endif /* JAPANESE_DOC */
END_HELP(kittygd)
...
" 参照: `pict2e`。",
"",
" (5) その他は、以下参照: `context`。および古い出力形式の以下も参照:",
" `texdraw`。",
"",
...
"",
" The default size for output is 5 inches x 3 inches. The `size` option",
...
" so the apparent font size is smaller than the nominal point size if the pdf",
" output is viewed at full size.",
"",
"    Examples :",
...
"",
" Pango, the library used to process the text, is based on utf-8.",
" The pdfcairo terminal will convert from your encoding to utf-8 if needed.",
" The assumed input encoding is taken from your \'locale\'. If your text actually",
" uses a different encoding, make sure gnuplot knows which one you are using.",
...
"",
" The rendering of the plot involves two mechanisms : antialiasing and",
" oversampling.  Antialiasing produces smoother non-horizontal and non-vertical",
" lines.  Oversampling combined with antialiasing provides subpixel accuracy,",
...
"                      {size <XX>{unit},<YY>{unit}}",
"",
" この出力形式は、ラベルやその他の文字列を、デフォルトで拡張文字列処理モ",
" ード (enhanced text mode) で処理します。以下参照: `enhanced`。",
"",
...
" のデフォルトは 0.5 なので、PDF 出力をフルサイズで見た場合、見かけのフ",
" ォントサイズは、名目上のポイントサイズよりも小さくなるでしょう。",
"",
"    例 :",
"       set term pdfcairo font \"Arial,12\"",
...
"",
" 文字列の処理を行うライブラリ Pango は、utf-8 に基づいているので、必要",
" なら pdfcairo 出力形式は、あなたの文字コードを utf-8 に変換します。入",
" 力で想定する文字コードはあなたが使用している \'locale\' から取るので、",
" あなたの文字列が実際には違う文字コードならば、あなたがどの文字コードを",
" 使っているのかを確実に gnuplot がわかるようにしてください。詳細は以下",
" 参照: `encoding`。",
"",
...
"",
" グラフのレンダリングは、アンチエイリアス、オーバーサンプリングの 2 つ",
" の機構を持っています。アンチエイリアスは、水平や垂直でない線をより滑ら",
" かにします。オーバーサンプリングは、アンチエイリアスと組でピクセルより",
" も小さいサイズでの精度を提供し、gnuplot が非整数座標の直線を書けるよう",
" になります。これは、対角方向の直線 (例えば 'plot x') が左右に揺れるの",
" を避けます。",
""
#endif /* JAPANESE_DOC */
END_HELP(pdfcairo)
...
"",
" デフォルトでは (`anchor`) 各グラフは、ウィンドウの左上の領域に上書きす",
" る形で描画します。これは、再描画することでの場所固定式アニメーション",
" (in-place animation) の作成や、`pause mouse` の間に矢印キーを使った疑",
" 似マウス操作を可能にします。",
" それに対しオプション `scroll` は、各グラフを現在のカーソル位置に描くこ",
" とで、その後のグラフスクロールをテキストを挟むことで行えるようにします。",
""
#endif /* JAPANESE_DOC */
END_HELP(sixelgd)
START_HELP(svg)
...
"?sixeltek",
" 書式:",
"    set terminal sixeltek {<fontsize>} {mono|color|colors <n>}",
"                          {size <x>,<y>} {anchor|scroll}",
"",
...
"",
" `anchor` は新しいグラフをそれぞれウィンドウの左上に置いていきますが、",
" `scroll` は各グラフを現在のカーソル位置に描き、テキストでスクロールで",
" きるようにします。",
"",
" xterm で利用する場合、\"--enable-sixel-graphics\" 付きで xterm が ",
" configure/コンパイルされていて、かつ \"-ti 340\" をつけて起動する必要",
" があります。",
"",
