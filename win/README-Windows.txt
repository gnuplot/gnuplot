...
wgnuplot text window and the windows terminal load and save settings from/to
wgnuplot.ini located in the appdata directory, see `help wgnuplot.ini`.
...
encodings supported by gnuplot, including UTF-8, see `help encoding`.  By
default gnuplot will use an encoding which matches the system's ANSI codepage,
...
in gnuplot. Below, we only list some important ones:
...
* Variable GNUPLOT_LIB may be used to define additional search directories for
  ...
  and `save set` commands. See 'help loadpath' for more details.
