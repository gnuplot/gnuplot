...
gnuplot バージョン 5.2 パッチレベル 0 -- Windows 用バイナリ配付
=======================================================================
...
* gnuplot.exe:  テキスト (コンソール) モード版の gnuplot 実行ファイルで
  ...
  代わるもので、例えば GNU Octave (www.octave.org) のように、gnuplot を
  グラフエンジンとして使用するような他のアプリケーションではこちらを使
  用することを推奨します。

* ランタイムライブラリファイル
  gnuplot が必要とするランタイムライブラリファイル (libfreetype-6.dll 等
...
Windows 版 gnuplot は3種類の対話型ターミナルを実装しています。windows, wxt,
qt です。wxt / qt ターミナルは他のプラットフォームでも実装されています。
3種類のターミナルはヒンティングを利用したアンチエイリアスとオーバーサン
プリングの技術により高画質の出力が可能であり、最近の gnuplot のすべての
実装をサポートしています。相違点は保存やクリップボードへのエクスポート
形式、印刷のサポート、パーシストモードの挙動です。wxt ターミナルは 
pngcairo や pdfcairo ターミナル（それらは非対話のグラフの保存を実現する
ものです。）と同じ描画コードを利用しています。windows ターミナルのグラ
フウィンドウは wgnuplot のテキストウィンドウとドックキングすることができ
ます。

デフォルトでは，Windows 版 gnuplot は wxt ターミナルを使用します。環境変数
GNUTERM を設定することでこれを変えることができます。後述の環境変数の設定
を参照してください。他の方法としては
    set term windows / wxt / qt
を gnuplot.ini に書き加えてください。`help startup` を参照してください。
...
* Windows のエクスプローラーの "ファイル名を指定して実行" のダイアログ
  ...
  Windows-R キーを押して `wgnuplot' を選択して実行することで wgnuplot を
  起動できるようになります。

* デフォルトの出力形式 (terminal) を wxt/windows/qt から選択すると、イン
  ストーラはそれに応じて GNUTERM 環境変数を更新します。
...
Windows 版バージョン 5.2 では、utf-8 を含むすべてのエンコーディングを
利用してコマンドライン入力をサポートしています。`help encoding`を参照し
てください。デフォルトでは、gnuplot はサポートしていればシステムの ANSI
コードページに適合するエンコーディングを使用します。`set encoding utf8`
を gnuplot.ini に加えることを推奨します。以下を参照。コマンドラインから
のユニコードの入力はベーシックマルチリンガルプレイン (BMP) に制約を受け
ますが，スクリプトはすべての文字を含むことができます (以前のバージョ
ンと同様に) 。
...
ォルトでは gnuplot は wgnuplot-XX.mnu と wgnuplot-XX.chm を読み込もう
とします。ここで、XX は 2 文字の言語コードです。現在のところ、英語 
(デフォルト) と日本語 (ja) をサポートしていますが、他の地域対応ファイル
も募集しています。
...
ルパネル]、[システム]、([詳細設定])、[環境変数] で、Windows 7/8/10 
ではデスクトップのコンピュータアイコンを右クリックして [プロパティ] を
選択し、[システム]、[システムの詳細設定]、[詳細設定]、[環境変数] で行
います。
