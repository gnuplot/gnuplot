...
%
% $Id: faq-ja.tex,v 1.38 2023-04-07 10:10:40+09 shige Exp $
% translated to Japanese by Shigeharu TAKENO
% Original: Id: faq.tex updated at 2023-04-05
%
%%%% \documentclass[a4paper,11pt]{article}
%%% \documentclass[letter,11pt]{article}
%
%\documentclass[a4paper,11pt]{jarticle} % for platex
\documentclass[a4paper,11pt]{article} % for lualatex
%% end Japanese
...
\usepackage{luatexja} % for lualatex
...
\ifx\pdfoutput\undefined
    ...
    \usepackage[
%        hypertex, % commented out for lualatex
        hyperindex,
        bookmarks,
...
\gnuplot は、MS Windows, linux, unix, macOS などの
...
ソースファイルのうち、個別に、明らかに二重ライセンスのものが
いくつかあります。その場合、その特定のファイルに含まれるコードの
再配布に関する代わりの条項は、そのファイルの先頭に挙げられています。
...
ソース配布物 ("gnuplot-5.4.0.tar.gz" または同様の名前) %
...
より活発なヘルプフォーラムが StackOverflow にあります。
\http{stackoverflow.com/questions/tagged/gnuplot}
...
%%% you must subscribe before you can post to it. 
...
そのメーリングリストが別に受けてしまう大量の spam を削減するため、
...
%%% processor can understand, and then you read in the file from your 
...
%%% Octave
%%% (\http{www.octave.org}), use this method. This also works from a 
%%% cgi script to
...
その順番は、コマンド {\em set pm3d depthorder} で制御できます。
...
\gnuplot バージョン 5.4 までは、直接アニメーションファイルを出力できる
出力形式は一つだけ (gif) でした:
%% end Japanese
\begin{verbatim}
...
%%% the same layout
%%% as a 2D graph made with {\em plot}, with the x axis horizontal and the
%%% y axis vertical.
%%% Version 5.4 commands {\em set view projection xz} and {\em set view 
%%% projection yz}
%%% similarly initialize layouts for 2D projection of the xz or yz plane, 
%%% with the
%%% z axis horizontal and the x or y axis vertical.
コマンド {\em set view map} は、視角とスケールを調整し、
それに続く {\em splot} による 3 次元グラフを、
{\em plot} による 2 次元グラフとほぼ同じレイアウトで、x 軸を水平に、
y 軸を鉛直方向に表示してくれます。
バージョン 5.4 のコマンド {\em set view projection xz} や 
{\em set view projection yz} は、同様に xz や yz 平面の 
2 次元射影用のレイアウトを初期化し、z 軸を水平にし、
x 軸や y 軸を鉛直方向に表示します。
...
バージョン 5.4: 描画スタイル \textit{with polygons} は、2 次元描画でも、
3 次元描画でも多角形形状 (三角形、四辺形、八角形、...) を処理できます。
%% end Japanese
...
古いバージョンで、それを行う最も良い方法は、
3 次元物体の各面を色付きの四辺形として、
以下のような形式のファイルを使って記述することです:
%% end Japanese
%%% \small
%%% \begin{verbatim}
%%% # triangle 1
...
また、各三角形は実際には 4 つの頂点を持ち、そのうちの 2 つが同一である
ことにも注意してください。
...
%%% The depthorder rendering in version 5 does not handle inter-penetrating
%%% facets; version 6 does somewhat better.
%%% For true 3D rendering you would be probably be better off using a 
%%% ray-tracing program.
gnuplot は 3 次元モデリングソフトではありませんので、
バージョン 5 の depthorder による描画では、面同士の貫通は処理しません。
バージョン 6 は多少は行います。
本当の 3 次元描画のためには、
...
%%% in which text markup is indicated by a set of special characters 
%%% embedded in the text.
バージョン 5 以降、\gnuplot はデフォルトで拡張文字列モード
(enhanced text) で、それは特殊文字を文字列内に埋めこむことによって
文字列のマークアップを示します。
...
する必要はないでしょう。
しかしそうでない場合は、出力のためには gnuplot に UTF-8 を
使用することを以下のように伝える必要があります:
{\em set encoding utf8}

%%% If you cannot enter UTF-8 characters on your keyboard you will have 
%%% to solve
%%% that outside of gnuplot. Or you can use octal escape sequences to 
%%% type them in
%%% byte-by-byte, or (since version 5.4) unicode escape sequences like
%%% {\em \verb&\U+221E&} ($\infty$).
%%% If your keyboard does generate UTF-8 but you don't know what 
%%% keystrokes would
%%% produce a specific character, there are probably suitable character
%%% selection
%%% apps for your desktop (e.g. KDE {\em kcharselect} or GNOME 
%%% {\em Character Map}).
UTF-8 文字列をあなたのキーボードで入力できない場合は、
gnuplot の外でそれを解決する必要があるでしょう。
またはその 8 進のエスケープ文字列を 1 バイト毎入力するか、
(バージョン 5.4 以降は) {\em \verb&\U+221E&} ($\infty$) のような
...
最も自然な方法は、UTF-8 フォントを使い、文字 $\hbar$ 
(Unicode {\verb&\U+210F&}) を直接入力することです。
...
%%% directly.  The very similar glyph at (Unicode code point 
%%% {\verb&\U+2609&} ; "sun")
%%% may be even better, but not many fonts provide it.
Planck 定数と同様、最も自然な方法は UTF-8 フォントを使い、文字 $\odot$ %
(Unicode {\verb&\U+2299&} ; "circled dot operator") %
を直接入力することです。
非常によく似たグリフ (Unicode コードポイント {\verb&\U+2609&} ; "sun") は
よりいいかもしれませんが、
それを提供するフォントは多くはありません。
...
この場合は、\verb+$\alpha_{3}$+ ($\alpha_{3}$) のような、
...
最初に、それが本当にバグなのか、それとも
ある {\em set} コマンドで翻すことができる特徴なのかを見極めてください。
...
レポートを送る前に、質問のバグが、現在のバージョンのリリース後以降の
より新しいソースで既に修正されていないかチェックしてみてください。
それらは、バグ tracker では "pending-fixed" とマークされています。
...
%%% The Julia language is designed for numerical analysis and 
%%% computational science.
%%% Gaston is a julia package that provides an interface to gnuplot 
%%% for graphical output.
\textbf{julia + Gaston} (\http{github.com/mbaz/Gaston.jl}) 
Julia は、数値解析と計算機科学用に設計された言語です。
Gaston は、グラフ出力用に gnuplot とのやりとりを提供する 
julia パッケージです。

%%% \subsection{What if I need special function that gnuplot doesn't have?}
\subsection{gnuplot が持たない特殊関数が必要な場合は}

%%% \Gnuplot version 6 provides a greatly expanded set of complex 
%%% special functions.
%%% If the one you need is not included, you may have to use another 
%%% program or write
%%% a custom plugin for gnuplot.
\gnuplot バージョン 6 では、複素特殊関数のとても広いセットを提供しています。
もしあなたが必要とするものが含まれていなければ、
他のプログラムを使うか、gnuplot 用にカスタムプラグインを書くか、
のいずれかが必要でしょう。
...
そのキャッシュの更新に時間がかかります。
これは、Windows や macOS ではよく起こるのですが、
...
%%% For some purposes \Gnuplot{} treats numbers less than 1.e-08 as being 
%%% zero. Thus if you are trying to plot a collection of very small numbers, 
...
\gnuplot は、ある目的のため 1.e-08 より小さい数を 0 として扱います。
...
%%% \subsection{When I {\em replot} or {\em resize} a multiplot I get 
%%% only a fragment}
\subsection{multiplot のグラフを {\em replot} や {\em resize} すると
一かけらしか出ません}
...
再生成するために十分な情報を保持するだけなので、
multiplot 全体を完全に再現するには、それを生成する完全なコマンド列を
スクリプトファイルに保存する必要があります。
そして、{\em load} することでその内容を \gnuplot に取り込めば、
あなたが必要とする回数だけ異なる出力形式、異なる出力ファイルに対して
再描画をすることができます。

%%% The same problem arises when you resize a multiplot displayed in an 
%%% interactive
...
同じ問題が、対話型出力形式で multiplot を resize した場合にも起こります。
それは、resize イベントは通常 replot を実行しているからです。
{\em qt} と {\em wxt} 出力形式は、ツールボックスウィジェット内に、
resize 時に replot することを抑制するトグルスイッチを提供しています。
{\em x11} 出力形式は、option {\em set term x11 noreplotonresize} を
提供しています。
...
よって {\em set} か {\em unset output} コマンドを使用するか、
異なる出力形式を選択する ({\em set term}) か、
または gnuplot を終了するまでは、そのファイルは完全ではなく、
外部のアプリケーションで使用できるようにはなっていません。
