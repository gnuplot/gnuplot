#ifndef lint
static char *RCSid = "$Id: wtext.c%v 3.50.1.13 1993/08/19 03:21:26 woo Exp $";
#endif
...
LRESULT CALLBACK _export WndParentProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam);
...
ReadTextIni(LPTW lptw)
{
	...
	if ( (p = GetInt(profile, &lptw->Origin.x)) == NULL)
		lptw->Origin.x = CW_USEDEFAULT;
	if ( (p = GetInt(p, &lptw->Origin.y)) == NULL)
		lptw->Origin.y = CW_USEDEFAULT;
	if ( (file != (LPSTR)NULL) && (section != (LPSTR)NULL) )
	  GetPrivateProfileString(section, "TextSize", "", profile, 80, file);
	if ( (p = GetInt(profile, &lptw->Size.x)) == NULL)
		lptw->Size.x = CW_USEDEFAULT;
	if ( (p = GetInt(p, &lptw->Size.y)) == NULL)
